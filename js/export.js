...
function renderTable() {
    ...
    currentReportData.forEach(row => {
        ...
        const statusClass = row.durum === 'GELDİ' ? 'text-green-600' : 'text-red-600';
        tr.innerHTML = `
            ...
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center font-bold ${statusClass}">${row.durum}</td>
        `;
...
function exportPDF() {
    ...
    const tableColumn = ["Tarih", "Servis", "Ogrenci", "Durum"];
    const tableRows = [];

    currentReportData.forEach(row => {
        tableRows.push([row.tarih, row.servis, row.ogrenci, row.durum]);
    });
